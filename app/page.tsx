...
function ToolResult({
	...
	return (
		<details
			className={`not-prose my-3 border-l-2 pl-3 font-mono ${isError ? "border-red-500" : "border-(--border)"}`}
		>
...
function UsageLine({ usage }: { usage: NonNullable<ChatMessage["usage"]> }) {
	...
	return (
		<div className="font-mono text-[11px] text-(--muted)">
			{s.promptTokens} in ({s.cacheRead} cached · {s.cacheWrite} new · {s.uncached} fresh) →{" "}
			{s.outputTokens} out
...
const Message = memo(function Message({
	...
	return (
		<div className={`flex flex-col gap-1 ${isUserBubble ? "items-end" : ""}`}>
			<div
				className={`${isUserBubble ? "bg-(--bubble-user) max-w-[80%] px-4 py-2 rounded-2xl" : ""} ${message.failed ? "opacity-50" : ""}`}
			>
	...
	return (
		<div className="flex flex-1 flex-col items-center bg-gray-50 font-sans">
			...
			<main className="flex w-full h-dvh max-w-3xl flex-col bg-white">
				...
				>
					{isEmpty && <h2 className="text-(--muted)">Welcome to use the chatbot!</h2>}
